...
### 
[Adicionar uma imagem ou GIF da tela inicial do projeto para impacto visual.]
...
| :--- | :--- | :--- |
| **Página Inicial** (`/`) | Apresentação do projeto, destaques dos gatinhos para adoção (slideshow) e estatísticas de sucesso (gatos adotados). 
| **Adoção** (`/adotar`) | Galeria completa dos gatos disponíveis, com **filtros avançados** (idade, sexo, temperamento) e pesquisa por nome. 
| **Formulário de Adoção** (`/adotar/:id/formulario`) | Processo de aplicação detalhado, coletando informações do adotante e o **termo de responsabilidade**. 
| **Voluntariado** (`/sejavoluntario`) | Cadastro para **Respadrinhos** com múltipla escolha de áreas de atuação: lar temporário, transporte, divulgação em redes sociais e feiras. 
| **Apoio e Ajuda** (`/comoajudar`) | Guia completo sobre formas de contribuição: doações financeiras (PIX/QR Code), doação de insumos (ração, areia) e apadrinhamento de custos veterinários.

| **Autenticação** | Login (`/entrar`) e Cadastro (`/cadastrar`) de usuários (adotantes e voluntários) com validação de email. 
| **Conta do Usuário** (`/conta`) | **Dashboard** para o usuário acompanhar o status de suas solicitações (adoção/voluntariado) e editar dados cadastrais.
| **Blog & Notícias** (`/blog`) | Seção para postar artigos sobre cuidados felinos, dicas de saúde e histórias emocionantes de resgate e adoção. 
| **Administração** (`/admin`) | Painel restrito para gestão de **usuários**, **gatos** (cadastro, edição, remoção), **solicitações** e **conteúdo do Blog/Eventos**.
| **Eventos** (`/events`) | Divulgação de feiras de adoção e eventos de arrecadação de fundos com data, local e mapa de acesso. 
...
| `/adotar/:id/formulario` | `pages/Quero-adotar` | Formulário de adoção específico |
